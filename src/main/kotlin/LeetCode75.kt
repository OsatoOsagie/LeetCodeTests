...
//        backtrackTiles(mutableListOf(), ans, tiles)
//        return ans.size
//    }
//
//    fun backtrackTiles(path: MutableList<Char>, ans: MutableSet<String>, tiles: String) {
//        // Add the current non-empty sequence
//        if (path.isNotEmpty()) {
//            println(path.joinToString(""))
//            ans.add(path.joinToString(""))
//        }
//
//        // Stop if the path uses all tiles
//        if (path.size == tiles.length) return
//
//        for (tile in tiles) {
//            // Ensure we don't use a tile more times than it appears in `tiles`
//            if (path.count { it == tile } < tiles.count { it == tile }) {
//                path.add(tile)
//                backtrackTiles(path, ans, tiles)
//                path.removeLast()
//            }
//        }
//    }
//
//    fun subarraySum(num: IntArray, k: Int): Int {
//        val ans = mutableListOf<List<Int>>()
//        backTrackSubarraySum(mutableListOf(),0,0,num,k,ans)
//
//        return ans.size
//
//    }
//
//    fun backTrackSubarraySum(path: MutableList<Int>, start: Int, curr: Int,nums: IntArray,k: Int,ans: MutableList<List<Int>>){
//
//        if (curr == k  && path.isNotEmpty()){
//            println(curr)
//            ans.add(ArrayList(path))
//            return
//        }
//
//        for (num in  start until nums.size){
//
//            if (curr+nums[num] <= k){
//                path.add(nums[num])
//                backTrackSubarraySum(path,num+1,curr+nums[num],nums,k,ans)
//                path.removeLast()
//            }
//        }
//
